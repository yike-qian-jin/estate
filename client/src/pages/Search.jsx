...
function Search() {
  ...
  useEffect(() => {
    ...
    const typeFromUrl = urlParams.get("type", sidebarParams.type);
    const parkingFromUrl = urlParams.get("parking", sidebarParams.parking);
    const offerFromUrl = urlParams.get("offer", sidebarParams.offer);
    const furnishedFromUrl = urlParams.get(
      "furnished",
      sidebarParams.furnished
    );
    const sortFromUrl = urlParams.get("sort", sidebarParams.sort);
    const orderFromUrl = urlParams.get("order", sidebarParams.order);
    if (
      searchTermFromUrl ||
      typeFromUrl ||
      parkingFromUrl ||
    ...
    <div className="flex flex-col md:flex-row">
      <div className="p-7 border-b-2 md:border-r-2 md:min-h-screen">
        <form onSubmit={handleSearchSubmit} className="flex flex-col gap-8">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <div className="flex gap-2">
              <input
                ...
                id="sale"
                className="w-5"
                checked={sidebarParams.type === "sale"}
                onChange={handleChange}
              />
              <span>Sale</span>
            </div>
          ...
          <button className="text-white uppercase bg-slate-700 p-3 rounded-lg hover:opacity-95">
            Search
          </button>
        </form>
      </div>
